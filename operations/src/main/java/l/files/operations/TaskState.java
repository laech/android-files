...
public abstract class TaskState {
  ...
  /**
   * Task is submitted but pending execution.
   */
  @AutoValue
  ...
  public static abstract class Running extends TaskState {
    ...
    public Success success(Time time) {
      return new AutoValue_TaskState_Success(task(), target(), time);
    }

    public Failed failed(Time time, List<Failure> failures) {
      return new AutoValue_TaskState_Failed(
          task(), target(), time, ImmutableList.copyOf(failures)
      );
    }
  }
