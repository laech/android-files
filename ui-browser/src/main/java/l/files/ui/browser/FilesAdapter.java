...
final class FilesAdapter extends StableAdapter<Object, ViewHolder>
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_TYPE_FILE) {
            return newFileViewHolder(inflater, parent);
        }
        if (viewType == VIEW_TYPE_HEADER) {
            return newHeaderViewHolder(inflater, parent);
        }
        throw new IllegalArgumentException(String.valueOf(viewType));
    }

    private FileViewHolder newFileViewHolder(
    ...
    private ViewHolder newHeaderViewHolder(
        LayoutInflater inflater,
        ViewGroup parent
    ) {
    ...
    public void selectAll() {
        Sequence<FileInfo> files =
            filterIsInstance(asSequence(items()), FileInfo.class);
    ...
    static int calculateCardContentWidthPixels(CardView card, int columns) {
        ...
        return (int) (
            (metrics.widthPixels -
                res.getDimension(R.dimen.files_list_space) * 2) / columns
