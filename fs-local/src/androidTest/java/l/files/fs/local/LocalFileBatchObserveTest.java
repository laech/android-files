...
public final class LocalFileBatchObserveTest extends FileBaseTest {
    ...
    public void testNotifiesChildrenChange() throws Exception {
        ...
        try (Closeable ignored = dir1().observe(NOFOLLOW, observer, consumer, 10, MILLISECONDS)) {
            ...
            verify(observer, timeout(20)).onBatchEvent(false, names(a, b));
            verify(observer, timeout(20)).onBatchEvent(false, names(a, b));
            verifyNoMoreInteractions(observer);
        }
    }
    ...
    public void testNotifiesSelfAndChildrenChange() throws Exception {
        ...
        try (Closeable ignored = dir1().observe(NOFOLLOW, observer, consumer, 10, MILLISECONDS)) {
            ...
            verify(observer, timeout(20)).onBatchEvent(true, names(child));

            child.setLastModifiedTime(NOFOLLOW, Instant.ofMillis(3));
            verify(observer, timeout(20)).onBatchEvent(false, names(child));
