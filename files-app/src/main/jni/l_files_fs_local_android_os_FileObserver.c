...
jint Java_l_files_fs_local_android_os_FileObserver_00024ObserverThread_init(
    JNIEnv* env, jobject object) {

  return (jint)inotify_init();
}

void Java_l_files_fs_local_android_os_FileObserver_00024ObserverThread_observe(
...
jint Java_l_files_fs_local_android_os_FileObserver_00024ObserverThread_startWatching(
...
void Java_l_files_fs_local_android_os_FileObserver_00024ObserverThread_stopWatching(
    JNIEnv* env, jobject object, jint fd, jint wfd) {
  inotify_rm_watch((int)fd, (uint32_t)wfd);
}

int Java_l_files_fs_local_android_os_FileObserver_init(JNIEnv* env) {
  jclass clazz;

  clazz = (*env)->FindClass(env, "l/files/fs/local/android/os/FileObserver$ObserverThread");
