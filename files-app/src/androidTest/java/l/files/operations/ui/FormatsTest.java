...
public final class FormatsTest extends TestCase {
    ...
    public void testFormatTimeRemaining() {
        ...
        assertEquals("0:10", formatTimeRemaining(0, SECONDS.toMillis(1), 11, 1).get());

        // total=120, processed=50, elapsed=50 sec, speed=1/sec
        assertEquals("1:10", formatTimeRemaining(0, SECONDS.toMillis(50), 120, 50).get());

        // total=11, processed=1, elapsed=1 min, speed=1/min, left=10 min
        assertEquals("10:00", formatTimeRemaining(0, MINUTES.toMillis(1), 11, 1).get());

        // total=11, processed=1, elapsed=1 hr, speed=1/hr, left=10 hr
        assertEquals("10:00:00", formatTimeRemaining(0, HOURS.toMillis(1), 11, 1).get());

        // total=2_000_000, processed=1000, elapsed=1 ms, speed=1_000_000/sec
        assertEquals("0:01", formatTimeRemaining(0, 1, 2_000_000, 1000).get());
    }
}
